...
Includes support for fallback models.
"""
...
class LiteLLMConfig(BaseConfig):
    ...
    max_tokens: int = 2048
    timeout: int = 60
    api_base: str = "https://ete-litellm.ai-models.vpc-int.res.ibm.com"
    api_version: str = ""


class LiteLLMModel(BaseLLM):
    """
    LiteLLM language model wrapper with fallback support.
    ...
    def _try_model(
        ...
        **kwargs,
        ...
        Returns:
            Union[Any, float]: (response) or (None) on failure.
        """
            ...
            if response_format is not None:
                try:
                    ...
                    if content_stripped.startswith('```'):
                        content_stripped = re.sub(r'^```(?:json)?\s*\n', '', content_stripped)
